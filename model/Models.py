...
def get_subsequent_mask(seq, dim=2):
    ...
    # diagonal = 0 means you allow q access itself
    # subsequent_mask = torch.triu(
    #     torch.ones((dim, len_s, len_s), device=seq.device, dtype=torch.uint8), diagonal=0).permute(1,2,0)
    subsequent_mask = torch.triu(
...
class Encoder_ST(nn.Module):
    ...
    def forward(self, event_loc, event_time, non_pad_mask):
        ...
        #for debugging: empty used tensor on CUDA
        del slf_attn_mask_subseq, slf_attn_mask_keypad, slf_attn_mask
...
class Transformer_ST(nn.Module):
    ...
    def forward(self, event_loc, event_time):
        ...
        enc_output, enc_output_temporal, enc_output_loc, enc_output_mark = self.encoder(event_loc, event_time, non_pad_mask)
