...
seed="${SEED:-1}"
model="${MODEL:-SMASH_GMM}"     # SMASH / SMASH_decouple / SMASH_GMM
mode="${MODE:-train}"               # train / debug / test
total_epochs="${TOTAL_EPOCHS:-200}"

dim="${DIM:-2}"                     # 1 / 2 / 3 / 4
dataset="${DATASET:-AutoIntHawkes2}"    # Earthquake / Crime / football / Gaussian / ...
batch_size="${BATCH_SIZE:-96}"

cuda_id="${CUDA_ID:-0}"
save_path="${SAVE_PATH:-./ModelSave/debug/}"

cond_dim="${COND_DIM:-36}"
estimator="${ESTIMATOR:-wsm}"       # wsm / mle / dsm
normalize_location="${NORMALIZE_LOCATION:-0}"
...
K_trig="${K_TRIG:-2}"
spatial_weight_looser_factor="${SPATIAL_WEIGHT_LOOSER_FACTOR:-0.0}"
